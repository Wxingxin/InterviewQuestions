...
- 这种模块化的方法允许**不同的特性**以**不同的速度**发展，可以更快的**标准化**一些特性，而不必等待**整个规范**的完成。
...
##### CSS 像素（cSSPixeI），CSS 像素可以被看作是逻辑像素的一种形式，用在 Web 端的。

- 它们被设计用来简化 Web 开发者的工作，使网页在不同显示设备上都能保持设计的一致性。
- 随着设备屏幕密度的增加，浏览器会自动处理 CSS 像素与物理像素之间的比例关系，确保网页元素在视觉上的大小保持一致。

##### PPI（PixelsPerInch）物理像素的密度（每英寸像素数量）
...
- DPI 主要用于描述打印机输出的精细度。
- 例如，一个高 DPI 值的打印机可以打印出更细致、更少见瑕疵的图像。

##### PPI 和 DPI 的区别是什么？

- DPI 主要用于打印领域，而 PPI 则主要用于屏幕显示领域。
- DPI 衡量的是墨水点的数量，PPI 衡量的是像素的数量。

> ### DPR : device pixel ratio
>
> DPR : device pixel ratio
> 口 2010 年，iPhone4 问世，不仅仅带来了移动互联网，还带来了 Retina 屏幕；口 Retina 屏幕翻译为视网膜显示屏，可以为用户带来更好的显示；
> 口 在 Retina 屏幕中，一个逻辑像素在长度上对应两个物理像素，这个比例称之为设备像素比（device pixel ratio）；口我们可以通过 window.devicePixelRatio 获取到当前屏幕上的 DPR 值；
...
- 这样，无论设备的物理像素密度如何，使用逻辑像素单位开发的界面都能保持相对一致的大小和视觉效果。
- 当然这个过程中还衍生出来很多不同的概念，比如 PPI（物理像素的密度，每英寸的物理像素数量），DPR（设备像素比，也就是一个逻辑像素对应的物理像素数量），DPI（每英寸打印点数，它主要应用于打印领域）。
