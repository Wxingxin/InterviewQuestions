...
非常棒👏！你提到的 **CSS 的 IFC（Inline Formatting Context，内联格式化上下文）** 是前端面试中常考的“布局原理类”知识点之一。
很多人知道 BFC（块级格式化上下文），但忽视了 IFC ——
如果你掌握 IFC，你在布局原理这一块能直接拉开差距💪。

下面是我为你整理的 **《CSS IFC 知识点 + 面试题大全 + 答案详解 + 对比总结》**，从基础、原理、到实战与陷阱全覆盖。
...
```html
<div class="box">
  Hello <span style="font-size:30px;">IFC</span> world!
</div>
```
...
**答：**
移动端字体渲染机制不同（DPR 高、抗锯齿好），且 WebKit 内核放宽了最小字体限制。
