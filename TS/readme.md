...
# 💯💯💯 `interface` 和 `type` 区别大全

# 🎯 一句话总结（面试快速回答）

> **`interface` 用来描述对象结构，可扩展；`type` 用来给任意类型起别名，更灵活，支持联合、交叉、条件类型等高级类型。**
...
```ts
type A = { x: number }
type A = { y: string } // ❌ 报错：重复标识符
```

## **区别二：继承方式不同**

### **interface 支持 extends interface 或 type（结构兼容）**
...
⚠️ 但 `type` 不支持 "继承" 语法，只能用 `&`。

---

## **区别三：type 更强大（可以表示任意类型）**
...
🔔 **interface 无法实现这些类型表达能力。**

---

## **区别四：interface 更适合 OOP 与类的结构描述**
...
类可以 implements 多个 interface，但不能 implements 类型别名的联合类型等复杂结构。

---

## **区别五：报错信息差异**

* `interface` 报错更语义化（适合结构描述）
* `type` 报错更具体（条件类型、范型更强）
...
* 要定义 **复杂类型**（联合、交叉、条件类型）
* 需要类型组合逻辑
* 类型不仅是对象（如 function/string/tuple）
...
### ✔ `type` 是类型别名，不创建新的类型，仅创建引用

编译后不会生成新结构。

### ✔ `interface` 是声明 + 合并机制

底层会进行多次合并（在大型项目中可能引起性能差异，但通常可忽略）

---

# 📌 五、面试官最喜欢听的专业回答

> **interface 更适合描述对象结构，支持声明合并和继承，适合公共 API；type 是类型别名，能表达任意类型结构，支持联合、交叉、条件等高级类型，灵活度比 interface 更大。一般情况下：对象结构使用 interface，复杂类型使用 type。**
...
| 映射类型              | ❌          | ✔               |
| 元组                | ❌          | ✔               |
| 用于类实现（implements） | ✔          | ✔（但不适用于复杂 type） |
| 适用方向              | API 结构、类对象 | 复杂类型逻辑          |

---

如果你愿意，我还能给你整理一个 **一句话背诵版** 或 **真实面试问答示例**，帮助你提分。
